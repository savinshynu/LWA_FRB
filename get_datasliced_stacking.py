"""
Collects 2 seconds of data around the expected time of arrival of FRB Pulse
Option to correct bandpass, flag RFI and dedisperse the data and write out the files 
into numpy (.npz format)
"""
...
def main(args):
    ...
    #directory to save image candidates
    outdir = "/data/local/savin/lwa/lwafiles/stack_slices/"
    ...
    for c,filename in enumerate(filenames):
        ...
        wfdata,_ = incoherent(freq, wfdata, tInt, dm, boundary='wrap', fill_value=np.nan)
        ...
        slicename = filename+'_'+os.path.splitext(os.path.basename(args.metafile))+ "_slice"
        slicename = os.path.join(outdir, slicename)
        print(slicename)
        np.savez(slicename, wfdat = wfdata[max(0,exp_samp-5000):min(exp_samp+5000,maxind),:], tdat = tdat[max(0,exp_samp-5000):min(exp_samp+5000, maxind)], freq = freq, dm = dm, ra = ra, dec = dec, source = sourceName, cfreq = cFreq, srate = srate, lfft = LFFT, tint = tInt, obstime = tStart.datetime, beamid = beam, tunID = tuning)
...
if  __name__ == "__main__": 
    ...
    parser.add_argument('-m', '--metafile', type=str, required = True, 
